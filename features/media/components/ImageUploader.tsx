...
export function ImageUploader({ vehicleId, images }: ImageUploaderProps): React.JSX.Element {
  ...
  return (
    <div>
      <label className="mb-2 block font-body text-xs font-medium uppercase tracking-wide text-titanium">
        Fotografías
      </label>
      ...
      {images.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-3">
          {images.map((image) => (
            <div
              key={image.id}
              className="relative h-20 w-20 overflow-hidden border border-titanium/20"
            >
              ...
              <button
                ...
                className="absolute right-0 top-0 bg-carbon/80 px-1.5 font-body text-xs text-alabaster"
      ...
      <input
        ...
        className="font-body text-sm text-titanium"
      />

      {isUploading && <p className="mt-2 font-body text-xs text-titanium">Subiendo...</p>}
      {error && <p className="mt-2 font-body text-xs text-ignition">{error}</p>}
    </div>
  );
}
